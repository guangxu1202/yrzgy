...
class CommonController extends Controller {
    public function _initialize(){
        ...
        //获取研修生园地
        $article = M("article");
        $researcher  = $article ->join("as a LEFT JOIN article_model AS b ON a.pk = b.article_id LEFT JOIN model AS c ON b.model_id = c.pk ")->field("a.pk,a.title,a.is_title_bold,a.title_color") ->order("a.custom_sort desc,a.update_time desc") ->where("c.pk = 12 and a.is_show = 1") -> limit("5") -> select();
        $this -> assign("researcher",$researcher);

        //获取专业委员会
        $Committee  = $article ->join("as a LEFT JOIN article_model AS b ON a.pk = b.article_id LEFT JOIN model AS c ON b.model_id = c.pk ")->field("a.pk,a.title,a.is_title_bold,a.title_color") ->order("a.custom_sort desc,a.update_time desc") ->where("c.pk = 14 and a.is_show = 1") -> limit("6") -> select();
